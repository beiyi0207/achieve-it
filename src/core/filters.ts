import type { Achievement, AchievementSource, Child, GroupBy, Settings, SortDirection, SortKey, Tag, TermDate } from '../types';
...
export type RecordFilter = {
  ...
  batchId?: string;
  /** Only records created by this source ("user" matches records with no source). */
...
export function isFilterActive(f: RecordFilter): boolean {
  return f.q.trim() !== '' || f.childIds.length > 0 || f.tagIds.length > 0 || f.templateIds.length > 0 || !!f.batchId || !!f.source || f.range !== 'all';
}
...
export function filterAchievements(list: Achievement[], f: RecordFilter, ctx: FilterContext): Achievement[] {
  ...
  return list.filter((a) => {
    ...
    if (f.source && (a.source ?? 'user') !== f.source) return false;
...
export function viewFromQuery(q: URLSearchParams, settings: Settings): RecordView {
  ...
  return {
    filter: {
      ...
      source: q.get('source') === 'claude' ? 'claude' : q.get('source') === 'user' ? 'user' : undefined,
...
export function queryFromView(v: RecordView, settings: Settings): URLSearchParams {
  ...
  if (v.filter.source) q.set('source', v.filter.source);
...
export function recordsHref(partial: Partial<RecordFilter> & { group?: GroupBy }): string {
  ...
  if (partial.source) q.set('source', partial.source);
